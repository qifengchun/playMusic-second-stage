...
class LyricDownload : public QObject
{
    ...
    Q_PROPERTY(QList<double> duration READ duration WRITE setDuration NOTIFY durationChanged)//持续时间
    Q_PROPERTY(QList<QString> singerName READ singerName WRITE setSingerName NOTIFY singerNameChanged)

    Q_PROPERTY(QString  lyric READ lyric WRITE setLyric NOTIFY lyricChanged)//歌词
    Q_PROPERTY(QString  showLyric READ showLyric WRITE setShowLyric NOTIFY showLyricChanged)//显示歌词
public:
