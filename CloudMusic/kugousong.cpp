...
KuGouSong::KuGouSong(QObject *parent):QObject(parent)
{
    ...
    network_request = new QNetworkRequest();				//发送请求一得到AlbumID（专辑）和FileHash(文件散裂)
    ...
    network_request2->setRawHeader("Cookie","kg_mid=233");//http协议中的cookie详细介绍
    //Cookie 是浏览器中用作存储介质的小块数据，并随每个请求发送到服务器。它们有助于会话管理、用户个性化和跟踪。
    network_request2->setHeader(QNetworkRequest::CookieHeader,2333);//对应于 HTTP Cookie 标头，包含一个 QList < QNetworkCookie > 表示要发送回服务器的 cookie。//将已知标头标头的值设置为值，覆盖任何先前设置的标头。  此操作还设置等效的原始 HTTP 标头。
